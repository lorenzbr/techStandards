...
get_section_text <- function(contents, pdf, standard, nb.pages, nb.pages.with.content, path) {
  ...
  if (nrow(contents) > 0) {
    ...
    ## last row in contents goes until end of the document
    contents$text_line_id_end[nrow(contents)] <- max(pdf$line_id)
    
    ## now all the text between two adjacent sections is combined and assigned to the previous section
    ## for a focal section take the text_line_id from the subsequent section minus 1. then take all strings
    ## from the pdf data.frame, paste it and write it into the corresponding line in the contents data.frame
    ## for loop over all rows specified in contents
